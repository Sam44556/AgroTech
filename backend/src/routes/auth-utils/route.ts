 ...
 */
router.get("/verification-status", protectRoute, async (req: Request, res: Response) => {
  try {
 ...
 */
router.post("/resend-verification", protectRoute, async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
 ...
 */
router.get("/profile-completion", protectRoute, async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
 ...
 */
router.get("/session-info", protectRoute, async (req: Request, res: Response) => {
  try {
    const session = await auth.api.getSession({
      headers: fromNodeHeaders(req.headers),
    });
