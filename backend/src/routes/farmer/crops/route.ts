...
router.get("/", farmerOnlyRoute, async (req: Request, res: Response) => {
  try {
    ...
    // Add computed fields
    const cropsWithStats = crops.map((crop: { orderItems: any[]; }) => ({
      ...crop,
      views: Math.floor(Math.random() * 200) + 10, // Mock data
      inquiries: Math.floor(Math.random() * 20) + 1, // Mock data
      totalSold: crop.orderItems.reduce((sum: any, item: { quantity: any; }) => sum + item.quantity, 0)
    }));
 ...
 */
router.get("/recent", farmerOnlyRoute, async (req: Request, res: Response) => {
  try {
    const farmerId = req.user!.id;
 ...
 */
router.put("/:id", farmerOnlyRoute, async (req: Request, res: Response) => {
  try {
    const farmerId = req.user!.id;
 ...
 */
router.delete("/:id", farmerOnlyRoute, async (req: Request, res: Response) => {
  try {
    const farmerId = req.user!.id;
    const cropId = req.params.id;
 ...
 */
router.patch("/:id/status", farmerOnlyRoute, async (req: Request, res: Response) => {
  try {
    const farmerId = req.user!.id;
    const cropId = req.params.id;
 ...
 */
router.get("/statistics", farmerOnlyRoute, async (req: Request, res: Response) => {
  try {
    const farmerId = req.user!.id;
    ...
    const topPerformingCrops = topCrops
      .map((crop: { name: any; orderItems: any[]; reviews: any[]; }) => ({
        name: crop.name,
